...
5. Run `moz-phab uplift --train <train> <base-rev> <tip-rev>`:
   ```bash
...
6. moz-phab submits the revision as **WIP (Changes Planned)** by
   default so you can review before Release Managers are notified.
   Open the lando URL printed at the end of the run
   (`https://lando.moz.tools/uplift/request/?revisions=<rev-id>`) and
   paste the Step 7 answer file into the nine-question uplift form —
   completing the form flips WIP off. (`--no-wip` on the moz-phab
   command also works but only do that after you've reviewed the
   revision.)
