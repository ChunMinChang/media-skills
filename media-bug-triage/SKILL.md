...
- **socorro-cli** — Query Mozilla's Socorro crash reporting system for crash volume, top crashes, and signature details. Use this
during triage to assess crash impact and validate signatures. (https://github.com/yjugl/socorro-cli)
- **crash-analysis** — A skill for deeper standalone crash investigation. Invoke this when a crash signature warrants full analysis
beyond what triage requires. (~/.claude/skills/crash-analysis/)

### Firefox Profiler Report Analysis

- **profiler-cli** — Fetch and parse Firefox performance profiles from profiler.firefox.com links shared in bug comments. Useful
when a reporter attaches a profile to demonstrate a performance regression or hang. (https://github.com/dpalmeiro/profiler-cli)
- **profiler-analysis** - A skill for extracting information from Mozilla Firefox performance profiles, which are generated by Firefox
for diagnosing issues. Can be useful in diagnosing playback and performance issues, as well as browser functionality issues through log analysis. (~/.claude/skills/profiler-analysis)

### Codebase Search

- **searchfox-cli** — Search the Firefox source code by identifier, symbol definition, or path. Use during codebase 
investigation (Step 6) to locate relevant files, function definitions, and recent changes. Run `searchfox-cli --help` for usage.

### Web Specification Status

- **webspec-index** - Tool for querying WHATWG, W3C, IETF and TC39 web specifications from the command line. (~/.claude/skills/webspec-index/)
